...
class HomeViewController: UIViewController {
    ...
    var menuItems: [UIAction] {
        return [
            UIAction(title: "Sign Out", image: UIImage(systemName: "sun.max"), handler: { (_) in
                self.signOut()
            }),
            UIAction(title: "Dark Mode", image: UIImage(systemName: "moon"), handler: { (_) in
                self.darkMode()
            })
        ]
    }
    ...
    override func viewDidLoad() {
        ...
        addGradient()
        fetchAll()
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Account", image: UIImage(systemName: "person"), primaryAction: nil, menu: menu)
